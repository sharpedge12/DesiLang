...
{
public:
	MakeTupleAction(const vector<Action> &sourceActionsIn) : ActionData(
																 [&]() -> Type
																 {
																 ...
																 Void, Void)
	{
		...
		{
			error.log("MakeTupleAction empty list ke sath create hua", INTERNAL_ERROR);
		}
		...
		for (auto i = sourceActions.begin(); i != sourceActions.end(); ++i)
		{
			...
			{
				error.log((*i)->getDescription() + " ko tuple creation mai daal diya but input void nhi hai", INTERNAL_ERROR);
			}

			if ((*i)->getReturnType()->matches(Void))
			{
				error.log((*i)->getDescription() + " ko tuple creation mai daal diya but output void hai", INTERNAL_ERROR);
			}
		}
	}
...
{
public:
	CppTupleCastAction(Action actionIn, Type returnType) : ActionData(returnType, Void, Void)
	{
		...
		{
			throw DesiLangError("CppCastAction ko design kra hai to cast matching tuples , but uska different use ho rha hai", INTERNAL_ERROR);
		}
	...
	void *execute(void *inLeft, void *inRight)
	{
		throw DesiLangError("CppCastAction interpreter se execute hue , jo galat hai ", INTERNAL_ERROR);
	}
...
{
public:
	GetElemFromTupleAction(Type typeInIn, string nameIn) : ActionData(typeInIn->getSubType(nameIn).type, typeInIn, Void)
	{
		typeIn = typeInIn;
...
Action getElemFromTupleAction(Type source, string name)
{
	if (!source->getSubType(name).type)
		throw DesiLangError("find nhi kr pae '" + name + "' in " + source->getString(), SOURCE_ERROR);

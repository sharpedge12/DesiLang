...
#define getdlcnType(typeIn) CONCAT(dlCN, typeIn)
...
#define dlCN_String String
#define dlCN_Dub Dub
#define dlCN_Int Int
#define dlCN_Byte Byte
#define dlCN_Bool Bool
#define dlCN_Void Void
// #define dlCN_Tuple(t1, t2) Type(new TypeData(vector<Type>({dlCN##_##t1, dlCN##_##t2}), ""))
#define dlCN_Tuple(t1, t2) makeTuple(vector<NamedType>({{"a", t1}, {"b", t2}}), true)
...
#define DO_RETURN_VAL(typeIn, varName)                        \
	void *outPtr = malloc(getdlcnType(typeIn)->getSize());    \
	memcpy(outPtr, &varName, getdlcnType(typeIn)->getSize()); \
	return outPtr;
...
#define func(nameText, leftType, rightType, returnType, lambdaBody, cpp)                                  \
	addAction(                                                                                            \
		nameText, getdlcnType(leftType), getdlcnType(rightType), getdlcnType(returnType), LAMBDA_HEADER { \
	...
	return [=](Action inLeft, Action inRight, CppProgram *prog)
	{
		...
		do
		{
			...
			else
			{
				...
				{
					throw DesiLangError("invalid '$' escape C++ code mai: " + cppCode, INTERNAL_ERROR);
				}
			}
...
inline T getValFromTuple(void *data, Type type, string name)
{
	...
	if (!a.type)
		throw DesiLangError("invalid property access krne ka try hua on '" + name + "' from type " + type->getString(), INTERNAL_ERROR);
...
inline T getValFromTuple(void* data, Type type, int index)
{
	if (index<0 || index>=type->getAllSubTypes()->size())
		throw DesiLangError("tried to get invalid property #"+to_string(index)+" from tuple "+type->getString(), INTERNAL_ERROR);
...
inline string dlcnStr2CppStr(void *obj)
{
...
inline void *cppStr2dlcnStr(string cpp)
{
...
void addToProgdlStr(CppProgram *prog)
{
	if (!prog->hasFunc("$dlStr"))
	{
		string strType = prog->getTypeCode(String);

		prog->addFunc("$dlStr", {{"const char *", "in"}}, strType,
					  "int size = 0;\n"
					  "while (in[size]) size++;\n"
...
void addToProgDoubleToStr(CppProgram *prog)
{
	if (!prog->hasFunc("$doubleToStr"))
	{
		...
		string intToStr = prog->dlToCpp("$intToStr");
		string concat = prog->dlToCpp("$concatStr");
		string dlStr = prog->dlToCpp("$dlStr");
		...
		prog->addFunc("$doubleToStr", {{"double", "in"}}, prog->getTypeCode(String),
					  ...
					  "while (b>0 && !(b % 10))\n\tb /= 10;\n"
					  "return " +
						  concat + "(" + concat + "(" + intToStr + "((int)in), " + dlStr + "(\".\")), " + intToStr + "(b));\n");
	}
}
...
void addToProgGetInputLine(CppProgram *prog)
{
	if (!prog->hasFunc("$getInputLine"))
	{
		...
		prog->addFunc("$getInputLine", {{strType, "prompt"}}, strType,
					  "fputs(" + prog->dlToCpp("$cStr") + "(prompt), stdout);\n"
														  "fflush(stdout);\n"
...
void addToProgRunCmd(CppProgram *prog)
{
	if (!prog->hasFunc("$runCmd"))
	{
		...
		prog->addFunc("$runCmd", {{strType, "cmd"}}, strType,
					  strType + " result = " + prog->dlToCpp("$dlStr") + "(\"\");\n"
																		 ...
																		 "	{\n"
																		 "		result=" +
						  prog->dlToCpp("$concatStr") + "(result, " + prog->dlToCpp("$asciiToStr") + "(c));\n"
																									 "	}\n"
																									 "}\n"
																									 "pclose(pipe);\n"
																									 "return result;\n");
...
void populateConstants()
{
	...
	addAction(
		...
		LAMBDA_HEADER {
			...
			{
				return cppStr2dlcnStr(cmdLineArgs[right]);
			}
			else
			{
...
void populateStdFuncs()
{
	...
	addAction(
		...
		LAMBDA_HEADER {
			cout << dlcnStr2CppStr(rightIn) << endl;
			return nullptr;
		},
		ADD_CPP_HEADER {
			addToProgCStr(prog);
		...
		AstWhatevToActionFactory::make(
			[](Type leftType, Type rightType) -> Action
			{
				...
				return lambdaAction(
					...
					{
						return cppStr2dlcnStr(val);
					},

					[=](Action inLeft, Action inRight, CppProgram *prog)
					{
						void *dlStr = cppStr2dlcnStr(val);
						constGetAction(dlStr, String, val, globalNamespace)->addToProg(prog);
						free(dlStr);
					},
					"typeName");
			}),
...
void populateStringFuncs()
{
	...
	addAction(
		...
		LAMBDA_HEADER {
			...
			if (val < 0 || val >= 256)
			{
				throw DesiLangError("ascii string mai se value nikalne ka try hua " + to_string(val), RUNTIME_ERROR);
			}
			string out;
			out += (char)val;
			return cppStr2dlcnStr(out);
		},
		ADD_CPP_HEADER {
			addToProgAsciiToStr(prog);
	...
	addAction(
		...
		LAMBDA_HEADER {
			...
			{
				throw DesiLangError("location access krne ka try hua for " + to_string(index) + " in string " + to_string(str.size()) + " long", RUNTIME_ERROR);
			}
			*out = str[index];
			return out;
		},
	...
	addAction(
		ops->plus, String, String, String,
		[=](void *leftIn, void *rightIn) -> void *
		{
			void *out = cppStr2dlcnStr(dlcnStr2CppStr(leftIn) + dlcnStr2CppStr(rightIn));
	...
	addAction(
		ops->equal, String, String, Bool,
		LAMBDA_HEADER {
			bool *out = (bool *)malloc(sizeof(bool));
			*out = dlcnStr2CppStr(leftIn) == dlcnStr2CppStr(rightIn);
			return out;
		},
		ADD_CPP_HEADER {
			addToProgEqStr(prog);
				...
				return lambdaAction(
					...
					[=](void *leftIn, void *rightIn) -> void *
					{
						...
						if (*(int *)rightIn < 0 || *(int *)rightIn >= size)
							throw DesiLangError("index out of bounds, element access krne ka try hua on " + to_string(*(int *)rightIn) + " in array " + to_string(size) + " long", RUNTIME_ERROR);
				...
				return lambdaAction(
					...
					[=](void *leftIn, void *rightIn) -> void *
					{
						...
						if (index < 0 || index >= size)
							throw DesiLangError("index out of bounds, tried to set element at position " + to_string(index) + " in array " + to_string(size) + " long", RUNTIME_ERROR);
...
void populateIntArrayAndFuncs()
{
	...
	addAction(
		"IntArray", Void, Int, IntArray, LAMBDA_HEADER {
	...
	addAction(
		"get", IntArray, Int, Int, LAMBDA_HEADER {
			int pos=*(int*)rightIn;
			int size=getValFromTuple<int>(leftIn, IntArray, "_size");
			if (pos<0 || pos>=size)
				throw DesiLangError("tried to acces position "+to_string(pos)+" of array "+to_string(size)+" long", RUNTIME_ERROR);
	...
	addAction(
		"set", IntArray, dlCN_Tuple(Int, Int), Void,
		LAMBDA_HEADER {
			Type rightType = dlCN_Tuple(Int, Int);
			int pos = getValFromTuple<int>(rightIn, rightType, "a");
			int size = getValFromTuple<int>(leftIn, IntArray, "_size");
			if (pos < 0 || pos >= size)
				throw DesiLangError("value access krne ka try hua on " + to_string(pos) + " of array " + to_string(size) + " long", RUNTIME_ERROR);
...
void populateNonStdFuncs()
{
	...
	addAction(
		...
		LAMBDA_HEADER {
			return cppStr2dlcnStr(runCmd(dlcnStr2CppStr(rightIn)));
		},
		ADD_CPP_HEADER {
...
{
	addAction(
		"cppCode", Void, String, Void, LAMBDA_HEADER { throw DesiLangError("'cppCode' ka use krke interpreter run nhi kr skte", SOURCE_ERROR); },
		ADD_CPP_HEADER {
			prog->pushBlock();
			addToProgStrWithEscapedNames(prog, dlcnStr2CppStr(right->execute(nullptr, nullptr)));
			prog->popBlock();
		});

	addAction(
		"cppHead", Void, String, Void, LAMBDA_HEADER { throw DesiLangError("'cppHead' ka use krke interpreter run nhi kr skte", SOURCE_ERROR); },
		ADD_CPP_HEADER {
			prog->addHeadCode(dlcnStr2CppStr(right->execute(nullptr, nullptr)));
		});
}

void populateDesiLangStdLib()
{
	basicSetup();
	populateBasicTypes();
	populateConstants();

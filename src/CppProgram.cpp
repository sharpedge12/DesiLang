...
string getValidCppId(string in)
{
	...
	do
	{
		...
		for (; i < int(in.size()) && ((in[i] >= 'a' && in[i] <= 'z') ||
									  (in[i] >= 'A' && in[i] <= 'Z') ||
									  (in[i] >= '0' && in[i] <= '9') ||
									  in[i] == '_');
			 i++)
...
void CppNameContainer::adddl(const string &dl, const string &cppNameHint)
{
	string validCppHint;

	if (cppNameHint == "<- dl string ki value")
		validCppHint = getValidCppId(dl);
	else if (!cppNameHint.empty())
		validCppHint = getValidCppId(cppNameHint);

	if (dlToCppMap.find(dl) != dlToCppMap.end())
	{
		throw DesiLangError(dl + "' ko dl name jese CppNameContainer mai dala but ye already exists krta hai", INTERNAL_ERROR);
	}

	// now we need to find a unique C++ name, the DesiLang name will almost always be unique, but there may be cases where DesiLang treats scope differently or uses a keyword or something
...
void CppNameContainer::reserveCpp(const string &cpp, bool ignoreCollisions)
{
	...
	{
		throw DesiLangError("CppNameContainer::reserveCpp ko id ke sath call kra'" + cpp + "', but ye already exist krta hai", INTERNAL_ERROR);
	}

	cppSet.insert(cpp);
}

bool CppNameContainer::hasdlMe(const string &dl)
{
	return dlToCppMap.find(dl) != dlToCppMap.end();
}

bool CppNameContainer::hasdl(const string &dl)
{
	return hasdlMe(dl) || (parent && parent->hasdl(dl));
}
...
string CppNameContainer::getCpp(const string &dl)
{
	auto result = dlToCppMap.find(dl);

	if (result == dlToCppMap.end())
	{
		if (parent)
			return parent->getCpp(dl);
		else
			throw DesiLangError("' " + dl + "' ka equivalent CppNameContainer::getCppFordl mai nhi mil paya", INTERNAL_ERROR);
	}
	else
...
void CppFuncBase::endln()
{
	if (exprLevel > 0)
	{
		throw DesiLangError("non zero expression level mila on ending line of C++ program:\n" + indentString(source), INTERNAL_ERROR);
	}
...
void CppFuncBase::popExpr()
{
	if (exprLevel <= 0)
	{
		throw DesiLangError("CppProgram::popExpression ko zero expressionLevel ke sath call kra", INTERNAL_ERROR);
	}
...
void CppFuncBase::pushBlock()
{
	if (exprLevel > 0)
	{
		throw DesiLangError("CppProgram::pushBlock ko call kra jaab expressionLevel zero tha", INTERNAL_ERROR);
	}
...
void CppFuncBase::popBlock()
{

	{
		if (blockLevel <= 0)
		{
			throw DesiLangError("CppProgram::popBlock ko zero indentationLevel ke sath call kra", INTERNAL_ERROR);
		}
...
string CppFuncBase::dlToCpp(const string &in)
{
...
{
	globalTopCode += "// this C++ code is transpiled from DesiLang\n";
	globalTopCode += "// DesiLang v" + to_string(VERSION_X) + "." + to_string(VERSION_Y) + "." + to_string(VERSION_Z) + " was used\n";
...
string CppProgram::getTypeCode(Type in)
{
	...
	case TypeBase::TUPLE:
	{
		...
		if (!globalNames->hasdl(compact))
		{
			globalNames->adddl(compact, in->nameHint);
			string code;
			code += "struct ";
	...
	default:
		throw DesiLangError("CppProgram::getTypeCode ko invalid type ke sath call kra " + (TypeBase::getString(in->getType())), INTERNAL_ERROR);
	}
}
...
void CppProgram::declareVar(const string &nameIn, Type typeIn, string initialValue)
{
	...
	*/
	...
	if (activeFunc->namespaceStack.back()->hasdl(nameIn))
	{
		return;
	}

	activeFunc->namespaceStack.back()->adddl(nameIn);
	activeFunc->namespaceStack[0]->reserveCpp(dlToCpp(nameIn), true);

	activeFunc->varDeclareSource += getTypeCode(typeIn) + " " + dlToCpp(nameIn);
...
{
	if (globalNames->hasdl(nameIn))
		return;

	string code;
	code += getTypeCode(typeIn);
	code += " ";
	globalNames->adddl(nameIn);
...
void CppProgram::addFunc(const string &name, vector<std::pair<string, string>> args, string returnType, string contents)
{
	if (hasFunc(name))
	{
		throw DesiLangError("CppProgram::addFunc ko  '" + name + "' ke sath call kra , jo already exist krta hai", INTERNAL_ERROR);
	}

	globalNames->adddl(name, name);
...
void CppProgram::pushFunc(const string &name, const string &cppNameHint, Type leftIn, Type rightIn, Type returnType)
{
	if (hasFunc(name))
	{
		throw DesiLangError("CppProgram::pushFunc ko function name '" + name + "' ke sath call kra, jo already exist krta hai", INTERNAL_ERROR);
	}

	globalNames->adddl(name, cppNameHint);
	...
	else
	{
		...
		for (int i = 0; i < int(args.size()); i++)
		{
			...
			funcNames->adddl(args[i].second, args[i].second);
...
void CppProgram::popFunc()
{
	...
	{
		throw DesiLangError("CppProgram::popFunc ko call kra jaab function ready nhi tha", INTERNAL_ERROR);
	}
	...
	if (funcStack.empty())
	{
		throw DesiLangError("CppProgram::popFunc ko kuch jyada baar call kra", INTERNAL_ERROR);
	}
...
{\n\
	argc = argcIn;\n\
	argv = argvIn;\n\
	if (argc >= 2 && strcmp(argv[1], \"--running-from-DesiLang\") == 0)\n\
	{\n\
		argc -= 2;\n\
		if (argc == 0)\n\
			argv = 0;\n\

...
void NamespaceData::addNode(AstNode node, string id)
{
	...
	else if (id=="__destroy__")
	{
		...
		{
			throw DesiLangError("incorrect type signiture for destroyer", SOURCE_ERROR, node->getToken());
		}
	...
	else if (id=="__copy__")
	{
		...
		{
			throw DesiLangError("incorrect type signiture for copier", SOURCE_ERROR, node->getToken());
		}
...
Type NamespaceData::getType(string name, bool throwSourceError, Token tokenForError)
{
	...
	if (results.empty())
	{
		if (parent)
			...
			throw DesiLangError("'" + name + "' type not found", SOURCE_ERROR);
		else
			return nullptr;
	}
	else if (results.size() != 1)
	{
		throw DesiLangError("namespace ke paas multiple definnitions hai iss type ka '" + name + "'", INTERNAL_ERROR);
	}
	else if (results[0]->getReturnType()->getType() != TypeBase::METATYPE)
	{
		throw DesiLangError("node jo non meta type hai vo stored in namespace type map for type '" + name + "'", INTERNAL_ERROR);
	}
	else
	{
...
Action NamespaceData::getDestroyer(Type type)
{
	...
	{
		throw DesiLangError("multiple destroyers for a single type in a single namespace", INTERNAL_ERROR);
	}
	else
	{
...
Action NamespaceData::wrapInDestroyer(Action in)
{
	...
	return destroyer ? branchAction(voidAction, destroyer, in)
					 : in;
}
...
Action NamespaceData::getCopier(Type type)
{
	...
	{
		throw DesiLangError("multiple copiers for a single type in a single namespace", INTERNAL_ERROR);
	}
	else
	{
		return nodes[0]->getAction();
...
Action NamespaceData::getActionForTokenWithInput(Token token, Type left, Type right, bool dynamic, bool throwSourceError, Token tokenForError)
{
	...
	if (!matches.empty())
	{
		...
		{
			throw DesiLangError("multiple matching instances of '" + token->getText() + "' found", SOURCE_ERROR, tokenForError);
		}
		else
		{
			return nullptr;
	...
	if (!matches.empty())
	{
		...
		{
			throw DesiLangError("multiple whatev instances of '" + token->getText() + "' found", SOURCE_ERROR, tokenForError);
		}
		else
		{
			return nullptr;
	...
	{
		if (foundNodes)
			throw DesiLangError("correct overload of '" + token->getText() + "' not found for types " + left->getString() + " and " + right->getString(), SOURCE_ERROR, tokenForError);
		else
			throw DesiLangError("'" + token->getText() + "' not found", SOURCE_ERROR, tokenForError);
	}
	else
	{
		return nullptr;

...
public:
	...
	void setInput(Namespace nsIn, bool dynamicIn, Type left, Type right)
	{
		if (inputHasBeenSet)
		{
			throw DesiLangError("AST node mai input set krna ka try hua '" + getString() + "' more then once", INTERNAL_ERROR, getToken());
		}
	...
	Type getReturnType()
	{
		if (!returnType)
		{
			if (!inputHasBeenSet)
			{
				throw DesiLangError("AST node ka return type access krna ka try kiya jaab input set nhi hua hai", INTERNAL_ERROR, getToken());
			}
		...
		if (!returnType)
		{
			throw DesiLangError("AST node " + getString() + "return type nhi de paya", INTERNAL_ERROR);
		}
	...
	void dealWithConstants()
	{
		if (!ns)
		{
			throw DesiLangError("constants se deal krna ka try kiya when input set nhi hai", INTERNAL_ERROR, getToken());
		}
...
public:
	...
	void resolveAction()
	{
		...
		{
			throw DesiLangError("AstVoid given non void input", INTERNAL_ERROR, getToken());
		}
...
public:
	static AstNode make(AstNode leftTypeIn, AstNode rightTypeIn, AstNode returnTypeIn, AstNode bodyIn)
	{
		...
		{
			throw DesiLangError("AstFuncBody bana with funtion input nodes that are not types", INTERNAL_ERROR);
		}
	...
	bool canBeWhatev()
	{
		...
		return leftTypeNode->getReturnType()->isWhatev() ||
...
public:
	...
	void resolveAction()
	{
		throw DesiLangError("AstType::resolveAction called, jo nhi hona chahiye", INTERNAL_ERROR, getToken());
	}
};
...
public:
	...
	void resolveAction()
	{
		throw DesiLangError("AstActionWrapper::resolveAction called, jo nhi hona chahiye", INTERNAL_ERROR);
	}
...
public:
	...
	void resolveAction() { throw DesiLangError("AstWhatevToActionFactory::resolveAction called, jo nhi hona chahiye", INTERNAL_ERROR); }

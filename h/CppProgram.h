...
// DesiLang names that are hardcoded should start with '-' so that collisions can't happen with input code
...
cars they can not collide with names from dl source and they will be removed before they get to C++ source
...
public:
	...
	void adddl(const string &dl, const string &cppNameHint = "<- the value of that dl string please"); // will throw an error if dlName already exists
	void reserveCpp(const string &cpp, bool ignoreCollisions = false);
	bool hasdl(const string &dl);
	string getCpp(const string &dl); // will throw an error if the DesiLang name doesn't exist
	CppNameContainer *getParent() { return parent; }

private:
	bool hasdlMe(const string &dl);
	CppNameContainer();
	...
	std::map<string, string> dlToCppMap;
...
public:
	...
	void name(const string &in); // converts a DesiLang name to a posibly different C++ name
	...
	void pushBlock();
	void popBlock();
	string dlToCpp(const string &in);
...
public:
	...
	string dlToCpp(const string &in) { return activeFunc->dlToCpp(in); }
